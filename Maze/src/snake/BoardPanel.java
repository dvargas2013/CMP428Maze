...
public class BoardPanel {
	...
	private static int EYE_LARGE_INSET = TILE_SIZE / 3;
	// The number of pixels to offset the eyes from the front.
	private static int EYE_SMALL_INSET = TILE_SIZE / 6;
	// The length of the eyes from the base (small inset).
	private static int EYE_LENGTH = TILE_SIZE / 5;
	...
	private void setTileSize() {
		TILE_SIZE = Math.min(game.getWidth(),game.getHeight())/25;
		EYE_LARGE_INSET = TILE_SIZE / 3;
		EYE_SMALL_INSET = TILE_SIZE / 6;
		EYE_LENGTH = TILE_SIZE / 5;
	}
	...
	// Sets the tile at the desired coordinate.@param point The coordinate of
	// the tile. @param type The type to set the tile to.
	...
	public void paint(Graphics g) {
		...
		g.fillRect(COL_COUNT*TILE_SIZE, 0, game.getWidth()-COL_COUNT*TILE_SIZE, game.getHeight());
		g.fillRect(0, ROW_COUNT*TILE_SIZE, game.getWidth(), game.getHeight()-ROW_COUNT*TILE_SIZE);
	...
	private void drawTile(int x, int y, TileType type, Graphics g) {
		...
		case SnakeBody:
			...
			break;
		// The snake head with two lines (representing eyes) that indicate it's
		// direction.
		case SnakeHead:
			 ...
			 * 
			 * First, we add (or subtract) EYE_SMALL_INSET to or from the side
			 * of the tile representing the direction we're facing. This will be
			 * constant for both eyes, and is represented by the variable
			 * 'baseX' or 'baseY' (depending on orientation).
			 * 
